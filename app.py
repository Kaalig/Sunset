...
def api_rdv_id(id):
    if request.method == 'GET':
        ...
        conn.commit()
        conn.close()
        return "Rendez-vous mis à jour", 200
    else:
        conn = sqlite3.connect('sunset.db')
        cursor = conn.cursor()
        ...
        conn.commit()
        conn.close()
        return "Rendez-vous supprimé", 200
